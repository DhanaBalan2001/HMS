  ...
  const handlePayment = async () => {
    ...
    try {
      if (selectedPaymentMethod === 'online') {
        ...
        const paymentResponse = await api.post('/appointments/payment-intent', {
          appointmentId: appointmentId
        });

        if (paymentResponse.data.success) {
          showToast('Processing online payment...');
          
          setTimeout(async () => {
            try {
              const confirmResponse = await api.post('/appointments/confirm-payment', {
                appointmentId: appointmentId
              });
              ...
              setPaymentProcessing(false);
              showToast('Payment confirmation failed. Please contact support.', 'error');
            }
          }, 2000);
        }
      } else {
        // Offline payment - just mark as pending
